    ...
    def do_POST(self):
        if self.path == '/api/chat':
            try:
                ...
                return self._send({'error': err_json.get('error', {}).get('message', f'HTTP {e.code}')}, e.code)
            except Exception as e:
            ...
            try:
                ...
                if not TAVILY_KEY:
                    return self._send({'error': '服务端未配置 TAVILY_KEY'}, 500)
                payload = json.dumps({
                    'query': query,
                    'search_depth': 'basic',
                    'include_answer': True,
                ...
                req.add_header('Content-Type', 'application/json')
                req.add_header('Authorization', 'Bearer ' + TAVILY_KEY)
...
.bar button.on{background:var(--red);color:#fff;border-color:var(--red)}
...
async function searchWeb(query, n){
  n=n||3;var results=[];
  try{
    var r=await fetch('/api/search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:query,max_results:n})});
  ...
  }catch(e){console.warn('Tavily search failed:',e.message);}
  if(!results.length){
    try{
